...
var pg = require('pg');
var client, willowClient, parcelClient, done;
...
function connect(callback) {
  ...
  var conString2 = `postgres://${ctConfig.username}:${ctConfig.password}@${ctConfig.host}:${ctConfig.port}/${ctConfig.db}`;
  pg.connect(conString2, function(err, c, d) {
    if( err ) {
      throw new Error(err);
    }
